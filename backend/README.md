...
`CORS_ORIGIN` and `LOG_LEVEL` are applied in `main.ts` before or outside typed `ConfigService` usage; other variables above match `src/config/env.validation.ts` where noted.
